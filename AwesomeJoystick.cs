...
public class AwesomeJoystick : MonoBehaviour
{
    ...
    */
	...
	private static List<AwesomeJoystick> joysticks;
	...
	private void Awake()
	{
		...
		if (!enumeratedJoysticks)
		{
			try
			{
				...
				joysticks = new List<AwesomeJoystick>(objs.Length);
				foreach (GameObject obj in objs)
				{
					AwesomeJoystick newJoystick = obj.GetComponent<AwesomeJoystick>();
					if (newJoystick == null)
					{
		...
		else
		{
			for (int i = 0; i < count; i++)
			{
				...
				if (shouldLatchFinger && (lastFingerId == -1 || lastFingerId != touch.fingerId))
				{
					...
					foreach (AwesomeJoystick j in joysticks)
					{
						if (j == this)
						{
							continue;
				...
				if (lastFingerId == touch.fingerId)
				{
                    ...
                    */
					...
					if (touchPad)
					{
						...
						position = new Vector2
							(
								Mathf.Clamp ((touch.position.x - touchZone.center.x), -1, 1),
								Mathf.Clamp ((touch.position.y - touchZone.center.y), -1, 1)
